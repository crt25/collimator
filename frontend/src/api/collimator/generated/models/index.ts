/**
 * Generated by orval v7.17.2 🍺
 * Do not edit manually.
 * Collimator
 * The Collimator API description (multi-version)
 * OpenAPI spec version: 1.0.0
 */

export * from "./anonymousStudentAuthenticationRequestDto";
export * from "./authenticationProvider";
export * from "./authenticationRequestDto";
export * from "./authenticationResponseDto";
export * from "./authenticationResponseDtoKeyPair";
export * from "./classStudentDto";
export * from "./classTeacherDto";
export * from "./classesControllerFindAllV0Params";
export * from "./classesControllerFindOneV0Params";
export * from "./classesControllerUpdateV0Params";
export * from "./copySessionDto";
export * from "./createClassDto";
export * from "./createKeyPairDto";
export * from "./createPrivateKeyDto";
export * from "./createReferenceSolutionDto";
export * from "./createSessionDto";
export * from "./createSolutionDto";
export * from "./createSolutionTestDto";
export * from "./createTaskDto";
export * from "./createUserDto";
export * from "./currentAnalysesDto";
export * from "./currentStudentAnalysisDto";
export * from "./deletedClassDto";
export * from "./deletedSessionDto";
export * from "./deletedTaskDto";
export * from "./deletedUserDto";
export * from "./deletedUserDtoPublicKeyId";
export * from "./errorCode";
export * from "./errorCodeDto";
export * from "./existingClassDto";
export * from "./existingClassExtendedDto";
export * from "./existingClassWithTeacherDto";
export * from "./existingSessionDto";
export * from "./existingSessionExtendedDto";
export * from "./existingSolutionDto";
export * from "./existingSolutionTestDto";
export * from "./existingSolutionTestDtoReferenceSolutionId";
export * from "./existingSolutionTestDtoStudentSolutionId";
export * from "./existingStudentSolutionDto";
export * from "./existingTaskDto";
export * from "./existingTaskWithReferenceSolutionsDto";
export * from "./existingUserDto";
export * from "./existingUserDtoPublicKeyId";
export * from "./isSessionAnonymousDto";
export * from "./keyPairDto";
export * from "./patchStudentActivityIsReferenceDto";
export * from "./patchStudentSolutionIsReferenceDto";
export * from "./privateKeyDto";
export * from "./publicKeyDto";
export * from "./referenceAnalysisDto";
export * from "./registrationTokenDto";
export * from "./sessionClassDto";
export * from "./sessionStatus";
export * from "./sessionTaskDto";
export * from "./sessionsControllerCopyV0Params";
export * from "./sessionsControllerFindAllV0Params";
export * from "./sessionsControllerFindOneV0Params";
export * from "./sessionsControllerFinishV0Params";
export * from "./sessionsControllerPauseV0Params";
export * from "./sessionsControllerStartV0Params";
export * from "./sessionsControllerUpdateV0Params";
export * from "./solutionsControllerDownloadLatestStudentSolutionV0Params";
export * from "./solutionsControllerDownloadOneV0Params";
export * from "./solutionsControllerFindCurrentAnalysesV0Params";
export * from "./solutionsControllerFindOneStudentSolutionV0Params";
export * from "./solutionsControllerPatchStudentActivityIsReferenceV0Params";
export * from "./solutionsControllerPatchStudentSolutionIsReferenceV0Params";
export * from "./studentActivityType";
export * from "./studentAuthenticationRequestDto";
export * from "./studentAuthenticationResponseDto";
export * from "./studentSessionProgressDto";
export * from "./studentTaskProgressDto";
export * from "./taskProgress";
export * from "./taskReferenceSolutionDto";
export * from "./taskType";
export * from "./tasksControllerDownloadOneV0Params";
export * from "./tasksControllerFindAllV0Params";
export * from "./tasksControllerFindOneV0Params";
export * from "./tasksControllerFindOneWithReferenceSolutionsV0Params";
export * from "./tasksControllerRemoveV0Params";
export * from "./tasksControllerUpdateV0Params";
export * from "./trackAppStudentActivityDto";
export * from "./trackAppStudentActivityDtoData";
export * from "./trackStudentActivitiesDto";
export * from "./trackStudentActivityDto";
export * from "./trackStudentActivityDtoAppActivity";
export * from "./updateClassDto";
export * from "./updateReferenceSolutionDto";
export * from "./updateSessionDto";
export * from "./updateTaskDto";
export * from "./updateUserDto";
export * from "./updateUserKeyDto";
export * from "./userType";
export * from "./usersControllerFindAllV0Params";
export * from "./usersControllerFindOneV0Params";
export * from "./usersControllerUpdateV0Params";
