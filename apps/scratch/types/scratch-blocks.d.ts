...
// Based on project: https://github.com/LLK/scratch-blocks

/// <reference path="../node_modules/@turbowarp/types/types/scratch-blocks.d.ts" />

// extend type definition
declare namespace ScratchBlocksExtended {
  const BROADCAST_MESSAGE_VARIABLE_TYPE: string;
  const SCALAR_VARIABLE_TYPE: VariableType;
  const LIST_VARIABLE_TYPE: VariableType;

  interface BlockFieldRow {
  ...
  class Flyout extends ScratchBlocks.Flyout {
    ...
    setCheckboxState(blockId: string, isVisible: boolean);
  }
  ...
  class Workspace extends ScratchBlocks.Workspace {
     ...
     * @param func The function to call when the
     ...
     * @param enabled Whether refreshes should be enabled.
     */
    setToolboxRefreshEnabled: (enabled: boolean) => void;

    setVisible: (visible: boolean) => void;

    updateToolbox: (toolboxXML: string) => void;

    glowStack: (id: string, shouldGlow: boolean) => void;
  ...
  interface Toolbox {
    ...
    getCategoryPositionById(id: string): number;
    getCategoryLengthById(id: string): number;
    setSelectedCategoryById(id: string): void;
    setFlyoutScrollPos(position: number): void;
  ...
  const Variables: {
     ...
     * @param workspace The workspace on which to create the
     *     variable.
     * @param opt_callback An optional callback function to act
     ...
     * @param opt_type Optional type of the variable to be created,
     *     like 'string' or 'list'.
     */
    createVariable: (
      workspace: Workspace,
  ...
  const Procedures: {
     ...
     * https://github.com/scratchfoundation/scratch-blocks/blob/2e3a31e555a611f0c48d7c57074e2e54104c04ce/core/procedures.js#L392
     * @param workspace The workspace to create the new procedure on.
     */
    createProcedureDefCallback_: (workspace: Workspace) => void;
  };

  const ScratchMsgs: {
    setLocale(locale: string): void;
    translate(key: string, input: string): string;
  };

  const Xml: {
    textToDom(xml: string): HTMLElement;
    clearWorkspaceAndLoadFromXml(dom: HTMLElement, workspace: Workspace);
  };

  /**
   * Inject a Blockly editor into the specified container element (usually a div).
   * @param container Containing element, or its ID,
   *     or a CSS selector.
   * @param Optional dictionary of options.
   ...
   * https://github.com/scratchfoundation/scratch-blocks/blob/2e3a31e555a611f0c48d7c57074e2e54104c04ce/core/field_colour_slider.js#L48
   * @param colour The initial colour in '#rrggbb' format.
   * @param opt_validator A function that is executed when a new
